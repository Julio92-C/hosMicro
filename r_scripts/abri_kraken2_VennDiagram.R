...
sample_category <- unique(abri_kraken2_filtered$sample)
taxa_category <- unique(abri_kraken2_filtered$name)

# Pivot the Sample values into columns
df_wide <- abri_kraken2_filtered %>%
  arrange(sample) %>%
  pivot_wider(names_from = sample, values_from = c(taxid), values_fn = sum) %>%
  group_by(name) %>%
  summarise(across(22:82, ~ paste(., collapse = ", "))) %>%
  mutate(across(2:62, ~ str_replace_all(., "(NA,|,NA|,NA,|NA|,| )", "")))
