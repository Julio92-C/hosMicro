...
# # The 50 most abundant taxa filtered
...
sample_category <- length(unique(abri_kraken2_filtered$sample))
taxa_category <- length(unique(abri_kraken2_filtered$name))
...
df_wide <- abri_kraken2_filtered %>%
  ...
  group_by(name) %>%
  summarise(across(22:75, ~ paste(., collapse = ", "))) %>%
  mutate(across(2:55, ~ str_replace_all(., "(NA,|,NA|,NA,|NA|,| )", "")))
...
ann_col_all <- inner_join(sample2arg_data_transposed, MetadataLocations, by = "sample") %>%
  na.omit() %>%
  remove_rownames() %>%
  ...
  rename(SAMPLE = type) %>%
  rename(WARD = ward) %>%
  select(SAMPLE, LOCATION, WARD)
  #select(SAMPLE, WARD)

# Generate a color palette based on the number of levels in location
d_palette <- paletteer_d("ggsci::schwifty_rickandmorty", n = nlevels(as.factor(ann_col_all$LOCATION)))
...
# Generate a color palette based on the number of levels in ward
w_palette <- paletteer_d("ggsci::default_jco", n = nlevels(as.factor(ann_col_all$WARD)))

# Create a named dataframe for the col colors
df5 <- ann_col_all %>%
  distinct(WARD) %>%
  mutate(color = w_palette)

# Create a named vector for rows
ward_colors <- setNames(as.character(df5$color), df5$WARD)

# pal_df <- paletter_dataframe
...
Resistance_category <- abri_kraken2_filtered %>%
...
ann_colors <- list(
  ...
  WARD = ward_colors,
  SAMPLE = c(Air = "#828385", Surface = "#ed8cb6"),
  DRUG = drug_colors,
...
heatmap_plot <- pheatmap(sorted_matrix, display_numbers = FALSE, cluster_cols = TRUE, cluster_rows = FALSE,
                         scale = "none", 
